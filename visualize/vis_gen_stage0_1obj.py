...
for i in range(pred_motion.shape[0]): # 每个seq遍历
    ...
    for k in range(length): ## 每帧遍历
        ...
        if 5*k >= seq_len:
            ...
            k_gt = 5*k
        # print(k)
        gt_obj_verts_k = gt_obj_verts[k_gt].numpy()
        pred_obj_verts_k = pred_obj_verts[k].numpy()

        pred_all_verts = np.vstack((gt_hand_verts[k_gt].numpy(),pred_obj_verts_k))
        gt_all_verts = np.vstack((gt_hand_verts[k_gt].numpy(),gt_obj_verts_k))
