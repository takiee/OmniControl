...
def vis_smpl(out_path, image, nf, vertices, faces, camera_R, camera_T, K = np.array([1031.8450927734375, 0.0, 932.1596069335938, 0.0, 1022.4588623046875, 541.9437255859375, 0.0, 0.0, 1.0]).reshape((3,3)),index=-1,face_index=-1):
...
for i in range(pred_motion.shape[0]):
    ...
    #         gt_obj_verts = obj_verts = torch.tensor(obj_verts).unsqueeze(0).repeat(goal_index,1,1).float()
    #         pred_obj_verts = obj_verts = torch.tensor(obj_verts).unsqueeze(0).repeat(60,1,1).float()
    #         gt_obj_pose = torch.tensor(obj_pose[:goal_index]).float()
    ...
    if args.stage == 'stage1_repair':
        ...
        if goal_index < 100:
            fake_goal_index = goal_index + 7
            obj_verts = torch.tensor(obj_verts).unsqueeze(0).repeat(fake_goal_index,1,1).float()
            hand_params = torch.tensor(np.load(join(seq_path,'mano/poses_right.npy')))[:fake_goal_index]
            obj_pose = torch.tensor(obj_pose[:fake_goal_index]).float()
            pred_trans = pred_motion[i,:fake_goal_index,:3]
            pred_theta = pred_motion[i,:fake_goal_index,3:]
            pred_rot = pred_motion[i,:fake_goal_index,3:6]
        else:
            obj_verts = torch.tensor(obj_verts).unsqueeze(0).repeat(60,1,1).float()
            hand_params = torch.tensor(np.load(join(seq_path,'mano/poses_right.npy')))[goal_index-59:goal_index+1]
            obj_pose = torch.tensor(obj_pose[goal_index-59:goal_index+1]).float()
    ...
    for k in range(len(gt_verts)):
        if k!= len(gt_verts)-1:
            continue
        img = np.zeros((2160, 3840,3), np.uint8) + 255
        # print(k)
        index = gt_verts[k].numpy().shape[0]
        face_index = hand_faces.shape[0]
        ...
        # pred_img, _ = vis_smpl(render_out, img, k, pred_all_verts, faces,camera_pose[:3,:3],camera_pose[:3,3],K,index,face_index)
        gt_img, _ = vis_smpl(render_out, img, k, gt_all_verts, faces,camera_pose[:3,:3],camera_pose[:3,3],K,index,face_index)
