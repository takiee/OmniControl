...
def main():
    ...
    for rep_i in range(args.num_repetitions):
        ...
        global_mean = torch.from_numpy(np.load('dataset/gazehoi_global_hand_obj_mean.npy'))
        global_std = torch.from_numpy(np.load('dataset/gazehoi_global_hand_obj_std.npy'))

        sample = sample * global_std + global_mean

        hand_motion = sample[:,:,:99]
        obj_motion = sample[:,:,99:]
...
def eval_all(res_path,all_hand_motions,all_obj_motions,seqs):
    ...
    for i in tqdm(range(num_samples)):
        ...
        ######## hand ################
        hand_params = torch.tensor(np.load(join(gt_seq_path,'mano/poses_right.npy'))).float()
        hand_trans = hand_params[:,:3]
        hand_rot = hand_params[:,3:6]
        hand_theta = hand_params[:,3:51]
        mano_beta = hand_params[:,51:]

        pred_hand_motion = torch.tensor(all_hand_motions[i])[:seq_len].float()
    ...
    traj_err_20 = (all_traj/total_traj_num).numpy()
