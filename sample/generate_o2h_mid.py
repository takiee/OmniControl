...
def main():
    ...
    if args.guidance_param != 1:
        print("Use Classifier-Free Model.")
    ...
    for rep_i in range(args.num_repetitions):
        ...
        # sample (B,30,291)
        sample = sample.permute(0, 3, 2, 1).squeeze(2).contiguous().cpu().numpy() 
        hand_motion = sample
        print(hand_motion.shape)

        gt_hand_motion = model_kwargs['y']['inp'].permute(0, 3, 2, 1).squeeze(2).contiguous().cpu().numpy() 

        # B,30,12
        obj_motion = model_kwargs['y']['obj_pose'].cpu().numpy() 
        print(obj_motion.shape)

        length = model_kwargs['y']['lengths'].cpu().numpy()

        
        all_hand_motions.append(hand_motion)
        all_hand_gt.append(gt_hand_motion)
        all_obj_motions.append(obj_motion)
        all_lengths.append(length)
        all_seqs.append(model_kwargs['y']['seq_name'])

    all_hand_motions = np.concatenate(all_hand_motions, axis=0)
    all_hand_gt = np.concatenate(all_hand_gt, axis=0)
    ...
    np.save(npy_path,
            {'all_hand_motions': all_hand_motions, 'all_hand_gt': all_hand_gt,'all_obj_motions':all_obj_motions, 'lengths': all_lengths, 'seqs':all_seqs})
    print("Save Done.")
